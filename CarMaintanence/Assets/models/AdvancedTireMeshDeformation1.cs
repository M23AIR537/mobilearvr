...
public class AdvancedTireMeshDeformation1 : MonoBehaviour
{
    ...
    public float neighborInfluenceRadius = 0.2f;
    ...
    public void Deform(float intensity)
    {
        ...
        // Precompute movement directions for large surface vertices
        Dictionary<int, Vector3> largeSurfaceMovements = new Dictionary<int, Vector3>();

        // First pass: Calculate movement for large surface vertices
        foreach (int vertexIndex in largeSurfaceVertices)
        {
            Vector3 falloffDirection = CalculateFalloffDirection(vertexIndex);
            Vector3 movementVector = falloffDirection * intensity * inflationAmount;

            if (boundaryVertices.Contains(vertexIndex))
            {
                movementVector *= boundaryConstraintFactor;
            }

            largeSurfaceMovements[vertexIndex] = movementVector;
            deformedVertices[vertexIndex] += movementVector;
        }

        // Second pass: Move smaller surfaces based on nearby large surface movements
        foreach (int vertexIndex in smallSurfaceVertices)
        {
            Vector3 weightedMovement = CalculateNeighborInfluencedMovement(vertexIndex, largeSurfaceMovements, intensity);

            if (boundaryVertices.Contains(vertexIndex))
            {
                weightedMovement *= boundaryConstraintFactor;
            }

            deformedVertices[vertexIndex] += weightedMovement;
        }
    ...
    Vector3 CalculateNeighborInfluencedMovement(int vertexIndex, Dictionary<int, Vector3> largeSurfaceMovements, float intensity)
    {
        Vector3 totalMovement = Vector3.zero;
        float totalWeight = 0f;

        // Find nearby large surface vertices and their influence
        foreach (int largeVertexIndex in largeSurfaceVertices)
        {
            // Calculate distance between vertices
            float distance = Vector3.Distance(
                originalVertices[vertexIndex],
                originalVertices[largeVertexIndex]
            );

            // Calculate influence based on distance
            if (distance <= neighborInfluenceRadius)
            {
                // Normalize distance for weight calculation
                float distanceWeight = 1f - (distance / neighborInfluenceRadius);

                // Get the movement of the large surface vertex
                Vector3 neighborMovement = largeSurfaceMovements[largeVertexIndex];

                // Weight the movement by distance and connectivity threshold
                totalMovement += neighborMovement * distanceWeight * connectivityThreshold;
                totalWeight += distanceWeight;
            }
        }

        // Fallback to normal scaling if no significant large surface influence
        if (totalWeight == 0f)
        {
            Vector3 referenceNormal = FindNearestLargeSurfaceNormal(vertexIndex);
            return referenceNormal * intensity * scaleFactor;
        }

        return totalMovement / totalWeight;
    }
